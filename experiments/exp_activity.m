...
data = data';
y = data(:, 1);
X = data(:, 2:547); % only take the first 20 features for testing
...
trainNdx = 1:floor(2*nSentences/3);
testNdx = floor(2*nSentences/3)+1:nSentences;
...
% 
[trainErr, prec] = crfChain_error(w,v_start,v_end,v,X,y,nStates,nFeatures,featureStart,sentences(trainNdx,:),'infer',useMex)
[testErr, prec] = crfChain_error(w,v_start,v_end,v,X,y,nStates,nFeatures,featureStart,sentences(testNdx,:),'infer',useMex)
weighted_prec  = sum(prec .* cls_weight) / length(y)
weighted_recall = sum(recall .* cls_weight) / length(y)
